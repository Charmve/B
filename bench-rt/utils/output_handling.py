...
def export_csv(data_directory, filename, data):
  ...
  with open(csv_file_path, 'w') as csv_file:
    ...
    csv_writer.writerow([
        'project_source', 'project_commit', 'bazel_commit', 'run', 'wall', 'cpu',
        'cpu_user', 'cpu_system', 'memory', 'command', 'expressions', 'hostname',
    ...
    for (bazel_commit, project_commit), data_item in data.items():
      ...
      for idx, run in enumerate(data_item['results'], start=1):
        csv_writer.writerow([
            ...
            idx, run['wall'], run['cpu'], run['cpu_user'], run['cpu_system'], run['memory'], command,
