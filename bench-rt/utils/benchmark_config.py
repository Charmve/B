...
class BenchmarkConfig(object):
    ...
    def get_bazel_commits(self):
        ...
        return [unit["bazel_commit"] for unit in self._units if "bazel_commit" in unit]
    ...
    def from_string(cls, string):
        ...
        benchmark_project_commits = (
            "benchmark_project_commits" in config
            and config["benchmark_project_commits"]
        )

        global_options = config["global_options"] if "global_options" in config else {}
    ...
    def _parse_unit(cls, unit):
        ...
        if "command" not in unit or not isinstance(unit["command"], str):
            raise ValueError(
                "A command has to be specified either as a global option"
        ...
        try:
            ...
            raise ValueError(
                "'%s' does not contain a Blaze command (e.g. build)" % unit["command"]
            )
        options = []
        ...
        while (
            full_command_tokens
            and full_command_tokens[0].startswith("--")
            and full_command_tokens[0] != "--"
        ):
