    ...
    def command(self, command, args=None):
        ...
        if self._platform == "j5":  # or 'x9' in self._platform:
            ...
            if command == "run":
                ...
                return None
            logger.log(
                "Executing Bazel command: %s %s %s %s %s"
                % (
                    self._bazel_binary_path,
                    " ".join(self._startup_options),
                    command,
                    self._platform,
                    args[2],
                )
            ...
            try:
                ...
                logger.log_error(
                    "QBuild command failed with exit code %s" % e.returncode
                )
            ...
            logger.log(
                "Executing Bazel command: bazel %s %s %s"
            ...
            try:
                ...
                exit_status = e.returncode
                logger.log_error(
                    "Bazel command failed with exit code %s" % e.returncode
                )
