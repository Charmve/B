...
def _prepare_time_series_data(raw_data):
    ...
    for row in raw_data:
        if row.project_label not in project_to_pos:
            ...
            headers.extend(
                [row.project_label, {"role": "interval"}, {"role": "interval"}]
            )
    ...
    return (
        [headers] + list(date_to_wall.values()),
        [headers] + list(date_to_mem.values()),
        project_to_pos.keys(),
    )
    ...
    links = " - ".join(
        [
            '<a href="https://perf.bazel.build/{project_label}/{date_subdir}/report.html">{project_label}</a>'.format(
                date_subdir=date.strftime("%Y/%m/%d"), project_label=label
            )
...
def _generate_report_for_date(
    date, storage_bucket, report_name, upload_report, bq_project, bq_table
):
