...
from testutils.fakes import FakeBazel, fake_exec_command, fake_log
...
class BenchmarkFunctionTests(absltest.TestCase):
    ...
    def test_setup_project_repo_exists(self, unused_chdir_mock, unused_exists_mock):
        with mock.patch.object(
            sys, "stderr", new=mock_stdio_type()
        ) as mock_stderr, mock.patch("benchmark.git.Repo") as mock_repo_class:
            mock_repo = mock_repo_class.return_value
            benchmark._setup_project_repo("repo_path", "project_source")

        mock_repo.git.fetch.assert_called_once_with("origin")
        self.assertEqual("Path repo_path exists. Updating...", mock_stderr.getvalue())

    @mock.patch.object(benchmark.os.path, "exists", return_value=False)
    @mock.patch.object(benchmark.os, "chdir")
    def test_setup_project_repo_not_exists(self, unused_chdir_mock, unused_exists_mock):
        with mock.patch.object(
            sys, "stderr", new=mock_stdio_type()
        ) as mock_stderr, mock.patch("benchmark.git.Repo") as mock_repo_class:
            benchmark._setup_project_repo("repo_path", "project_source")
        ...
        mock_repo_class.clone_from.assert_called_once_with(
            "project_source", "repo_path"
        )
        self.assertEqual(
            "Cloning project_source to repo_path...", mock_stderr.getvalue()
        )
    ...
    def test_get_commits_topological_latest(self):
        with mock.patch.object(
            ...
            result = benchmark._get_commits_topological(
                None, mock_repo, "bazel_commits"
            )

        self.assertEqual(["A"], result)
        self.assertEqual(
            "No bazel_commits specified, using the latest one: A",
            mock_stderr.getvalue(),
        )

    @mock.patch.object(benchmark.os.path, "exists", return_value=True)
    @mock.patch.object(benchmark.os, "makedirs")
    def test_build_bazel_binary_exists(self, unused_chdir_mock, unused_exists_mock):
        ...
        self.assertEqual(
            "Binary exists at outroot/commit/bazel, reusing...", mock_stderr.getvalue()
        )
...
class BenchmarkFlagsTest(absltest.TestCase):
    ...
    def test_project_source_present(self):
        ...
        with mock.patch.object(
            sys, "stderr", new=mock_stdio_type()
        ) as mock_stderr, self.assertRaises(SystemExit) as context:
