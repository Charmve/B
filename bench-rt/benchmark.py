...
def _build_bazel_binary(commit, repo, outroot, platform=None):
    ...
    """
    outroot_for_commit = (
        "%s/%s/%s" % (outroot, platform, commit)
        if platform
        else "%s/%s" % (outroot, commit)
    )
...
def json_profile_filename(
    ...
    return (
        f"{data_directory}/{bazel_bench_uid}_{bazel_commit}_{unit_num}"
...
def _single_run(bazel_bin_path, command, platform, options, targets, startup_options):
    ...
    if measurements != None:  # noqa: E711
        logger.log(
            "Results of this run: wall: "
            + "%.3fs, cpu %.3fs, cpu_user %.3fs, cpu_system %.3fs, memory %.3fMB, exit_status: %d"
            % (
                measurements["wall"],
                measurements["cpu"],
                measurements["cpu_user"],
                measurements["cpu_system"],
                measurements["memory"],
...
def _run_benchmark(
    ...
    logger.log(
        "=== BENCHMARKING %s for %s [Unit #%d]: %s, PROJECT: %s ==="
    ...
    if prefetch_ext_deps:
        ...
        _single_run(
            bazel_bin_path, command, platform, options, targets, startup_options
        )
    ...
    for i in range(1, runs + 1):
        ...
        if collect_profile and platform != "j5":
            assert bazel_identifier, (
                "bazel_identifier is required when " "collect_profile"
            )
            assert project_commit, "project_commit is required when " "collect_profile"
        ...
        collected.append(
            _single_run(
                ...
                command,
                platform,
                maybe_include_json_profile_flags,
                targets,
                startup_options,
            )
        )
...
def create_summary(data, project_source):
    ...
    for (i, bazel_commit, project_commit), collected in data.items():
        header = (
            "[Unit #%d] Bazel version: %s, Project commit: %s, Project source: %s"
        ...
        for metric, values in collected.items():
            # print("11111====> collected[%s]" % metric, values.values())
            ...
            if last_collected:
                ...
                mean_diff = "(% +6.2f%%)" % (
                    100.0 * (values_exclude_failures.mean() - base.mean()) / base.mean()
                )
                median_diff = "(% +6.2f%%)" % (
                    100.0
                    * (values_exclude_failures.median() - base.median())
                    / base.median()
                )
                print(
                    "====> base:%s, pval:%s, mean_diff:%s, median_diff: %s"
                    % base.values(),
                    pval,
                    mean_diff,
                    median_diff,
            ...
            summary_builder.append(
                ...
                % (
                    ...
                    (
                        "% 8.3f%s %s"
                        % (values_exclude_failures.mean(), unit[metric], mean_diff)
                    ).center(20),
                    (
                        "% 8.3f%s %s"
                        % (values_exclude_failures.median(), unit[metric], median_diff)
                    ).center(20),
                    (
                        "% 7.3f%s" % (values_exclude_failures.stddev(), unit[metric])
                    ).center(10),
                    pval.center(10),
                )
            )
...
flags.DEFINE_string(
    "bazel_bin_dir", None, "The directory to store the bazel binaries from each commit."
)
...
flags.DEFINE_list(
    "project_commits", None, "The commits from the git project to be benchmarked."
)
flags.DEFINE_string(
    "env_configure", None, "The shell commands to configure the project's environment."
)
...
# Miscellaneous flags.
flags.DEFINE_boolean("verbose", False, "Whether to include git/Bazel stdout logs.")
...
flags.DEFINE_string(
    "data_directory", None, "The directory in which the csv files should be stored."
)
...
# properly filled.
flags.DEFINE_string(
    "csv_file_name", None, "The name of the output csv, without the .csv extension."
)
...
def _flag_checks():
    ...
    if (
        not FLAGS.benchmark_config
        and FLAGS.bazel_commits
        and FLAGS.project_commits
        and len(FLAGS.bazel_commits) > 1
        and len(FLAGS.project_commits) > 1
    ):
        raise ValueError(
    ...
    if FLAGS.aggregate_json_profiles and not FLAGS.collect_profile:
        raise ValueError(
            "--aggregate_json_profiles requires " "--collect_profile to be set."
        )
...
def _get_benchmark_config_and_clone_repos(argv):
    ...
    if FLAGS.benchmark_config:
        ...
        bazel_clone_repo = _setup_project_repo(
            BAZEL_CLONE_BASE_PATH + "/" + _get_clone_subdir(bazel_source), bazel_source
        )
    ...
    bazel_clone_repo = _setup_project_repo(
        PROJECT_CLONE_BASE_PATH + "/" + _get_clone_subdir(bazel_source), bazel_source
    )
...
def main(argv):
    ...
    (
        ...
        bazel_clone_repo,
        project_clone_repo,
    ) = _get_benchmark_config_and_clone_repos(argv)
    ...
    for unit in config.get_units():
        if "bazel_binary" in unit:
            ...
            bazel_bin_path = subprocess_popen("which bazel")[
                0
            ]  # _exec_command is optional
    ...
    for i, unit in enumerate(config.get_units()):
        bazel_identifier = (
            unit["bazel_commit"] if "bazel_commit" in unit else unit["bazel_binary"]
        )
    ...
    if FLAGS.data_directory:
        ...
        if FLAGS.aggregate_json_profiles:
            aggr_json_profiles_csv_path = "%s/%s" % (
                FLAGS.data_directory,
                DEFAULT_AGGR_JSON_PROFILE_FILENAME,
            )
