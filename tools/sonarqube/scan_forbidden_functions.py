...
:2021 zhangwei
:changelogs
    1. 20220903 zhangwei  add function: search_target_2
    2. 20221012 zhangwei default_keywords中增加["printf", "puts", "cout", "cerr"]
    3. 20221013 zhangwei （1）修改search_target中的打印格式（2）扫描出问题时，不退出，返回问题的数量与扫描成功失败
    4. 20221014 zhangwei default_keywords中增加["fputs","putc","putchar","clog","perror"]
    5. 20221026 zhangwei 增加“check_specified_file_info”相关内容的检查
    6. 20221109 zhangwei 增加白名单scan_allow_list_file的功能
    7. 20221122 zhangwei search_target的正则匹配中，正价“::{}(”
"""
import argparse
import csv
import json
...
# Add more, ref: http://staff.ustc.edu.cn/~sycheng/sst/lectures/ch08_Secure_Programming.pdf
...
def search_target(afile, filter_path_list, scan_allow_dict, keyword):
    ...
    with open(afile, "r", encoding="utf-8") as f:
        file_content = f.read()
        # pattern = "{}\(.*\)|::{}::|::{}".format(keyword, keyword, keyword)
        pattern = r"regex({keyword}\(.*\)|::{keyword}::|::{keyword})"
...
def check_main(para_dict):
    ...
    filter_path_list.append("sample")
    filter_path_list.append("test")
    ...
    print("filter_path_list={}".format(filter_path_list))
    ...
    # check mlog 配置文件
    # [zhangwei] check_mlog_json(para_dict)

    # step 03> Judgment threshold
    expect_res = 0
    if res > expect_res:
        print("Please correct the forbidden function !!!")
        if param_dict["continue_on_err_gitlab"] != "true":
            print(
                "continue_on_err_gitlab={}".format(param_dict["continue_on_err_gitlab"])
            )
            sys.exit(1)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--continue_on_err_gitlab",
        help="Continue with errors for gitlab",
        default="false",
    )
    parser.add_argument(
    ...
    param_dict["continue_on_err_gitlab"] = args.continue_on_err_gitlab
