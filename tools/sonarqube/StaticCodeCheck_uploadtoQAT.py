...
"""
 ...
 @changelog:
    ...
    2. 20211109 zhangchengyu 增加白名单scan_allow_list_file的功能
"""
import argparse
import csv
import logging
...
class UploadToQAT(object):
    def __init__(self):
        ...
        self.continueOnError = "FALSE"  # 可选项, ["TRUE", "FALSE"]
        self.name_dict_path = "name_dict_info.csv"
    ...
    def analytical_parameters():
        ...
        parser.add_argument(
            "--continueOnError", help="Continue with errors", default="FALSE"
        )
    ...
    def _upload_dict(self, param_dict, result, threshold, report_link, info=None):
        ...
        self.post_to_backend(
            post_json=res_dict, endpoint=param_dict["static_check_result_endpoint"]
        )
        if (result == self.failed) and (continue_on_error != "TRUE"):
            # 20211111 add by zhangchengyu 目前parasoft不开门禁
            if info == "parasoft":
                logging.error("{} failed, please check pipeline!!!".format(info))
            else:
                self.except_exit(
                    info="{} failed, please check pipeline!!!".format(info)
                )
    ...
    def _execut_scan_forbidden_functions(
        ...
        """
        try:
            import scan_forbidden_functions
    ...
    def _execut_parasoft(self, result, threshold, report_link, param_dict, type_info):
        ...
        """
        try:
            import analyze_parasoft_report
    ...
    def _execut_unittest_and_coverage(self, param_dict):
        ...
        try:
            ...
            self.post_to_backend(
                post_json=res_dict,
                endpoint=param_dict["unittest_check_result_endpoint"],
            )
            if continue_on_error != "TRUE":
                if (lines == "none") or (functions == "none") or (branches == "none"):
                    ...
                    line_num = float(lines.split("%")[0])
                    functions = float(functions.split("%")[0])
                    branches = float(branches.split("%")[0])
    ...
    def _analytical_coverage_info(param_dict):
        ...
        branches = "none"
        with open(
            param_dict["coverage_log_path"], "r", encoding="utf-8", errors="ignore"
        ) as f_logs:
    ...
    def upload_to_qat(self, param_dict):
        ...
        if param_dict["check_type"] == "scan_forbidden_functions":
            ...
            link_lst = [
                ...
                "http://10.3.21.11:6959/sonarqube/scan_forbidden_functions.py",
            ]
...
if __name__ == "__main__":
    ...
    link_lst = [
        "http://10.3.21.11:6959/sonarqube/name_dict_info.csv",
        "http://10.3.21.11:6959/parasoft/check_code/analyze_parasoft_report.py",
    ]
